...
import Interview from "../models/interview_models.js";
...
export const analyzeResume= async(req,res)=>{
    try{
        ...
        const uint8Array=new Uint8Array(fileBuffer) //Uint8Array mein convert kar rhi h
        const pdf=await pdfjsLib.getDocument({data:uint8Array}).promise;
        let resumeText="";
        //Extract text from all pages
        for(let pageNum=1;pageNum<=pdf.numPages;pageNum++){
            const pages=await pdf.getPage(pageNum);
...
export const generateQuestion= async(req,res)=>{
    try{
        let {role, mode, experience, projects, skills, resumeText }=req.body;
        role=role?.trim();
    ...
    const interview=await Interview.create({
        ...
        questions:questionsArray.map((q,index)=>({
            question:q,
            difficulty:["easy","easy","medium","hard"][index],
            timeLimit:[60,60,90,120][index],
        }))
    })
